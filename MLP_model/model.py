...
def create_derivative_features_(df,num_vol=1):
    ...
    X = np.ones((df.shape[0],2*df.shape[1]+1),dtype=np.float32)*num_vol/200
    l = list(df.columns)
    for col in range(df.shape[1]):
        X[:,1+col] = df[l[col]].to_numpy()
        X[1:,1+df.shape[1]+col] = np.diff(X[:,1+col],axis=0)
        X[0,1+df.shape[1]+col] = X[1,1+df.shape[1]+col]
    return X

def data_preprocess(flight,flight_num):
    """ data preprocessing : 
...
class model_MLP(torch.nn.Module):
    """ MLP model to train that inherit from Pytorch Module class """
    def __init__(self):
        super(model_MLP, self).__init__()        
        self.fc1 = torch.nn.Linear(159, 2048,bias=False) 
        self.fc2 = torch.nn.Linear(2048, 256,bias=False)
        self.fc3 = torch.nn.Linear(256, 67,bias=False)   
        self.LKrelu = torch.nn.LeakyReLU(0.1)
...
class model:
    def __init__(self):
        ...
        self.torch_model_MLP = model_MLP()
        self.loss_func = F.mse_loss
        self.opt = torch.optim.Adam(self.torch_model_MLP.parameters(), lr=0.001)
        self.batch_size = 1024
        self.epoch_num = 10
        self.max_size = 1500000
    ...
    def fit(self, flights):
        ...
        for epoch in range(self.epoch_num):
            ...
            # accumulate the samples of some number of flights untils reaching the maximum size of an array (1.5 M of samples)
            i = 0
            while (i<len(flights)):
                i0 = i
                s = [0]
                ...
                df_input0 = np.zeros((s[-1],159),dtype=np.float32)
                ...
                for j in range(i0,i1):
                    ...
                    if flight_name not in self.missing_flight_list:
                        ...
                        df_input = data_preprocess(df_input,int(flight.name[0:-3]))
                        df_input = df_input.astype(np.float32)                                        

                        # fill missing output value with linear interpolation and bachward fill method
                ...
                for xb, yb in train_dl:                                    
                    pred = self.torch_model_MLP(xb)
                    pred = pred*yb[:,len(self.output_dict):]#tensor_mask 
    ...
    def predict(self, flight, outputs=None):
        ...
        try:
            ...
            for var in input_miss:
                df_input.insert(self.input_dict[var],var,np.zeros((df_input.shape[0])))
            df_input = data_preprocess(df_input,num_flight)
            df_input = df_input.astype(np.float32)
